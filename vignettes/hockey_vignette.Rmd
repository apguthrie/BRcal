...
Let's look at a few examples of how the hockey probabilities can be LLO-adjusted under different settings of `delta` and `gamma`.  Rather than stare at how the raw probabilities have changed under LLO-adjustment, we'll plot the original `x` vs the adjusted `x`.  

In the first example, we'll LLO-adjust the hockey predictions using `delta=2` and `gamma=2`.  Note that the points are the actual observations and the curve represents how any prediction along the 0-1 range would behave under `delta=2` and `gamma=2`.
...
To get a better feel for how different values of `delta` and `gamma` affect probabilities, let's plot a few more different adjustments on the same plot.
...
ggplot() +
  ...
  # geom_point(aes(color=winner), alpha=0.75, size=2) +
  lims(x=c(0,1), y=c(0,1)) +
  labs(x="Original", y="LLO(x, 2, 2)") +
  theme_bw()
```

- show what happens when you use really extreme values (force warning message) ? 

As previously mentioned, using extreme values of `delta` or `gamma` can cause instabilities.  For example, let's see what happens when
...
It's a little hard to see what `z` looks like in the above output. Below shows that `z` is a 200 by 200 matrix. The rows and columns are named based on the value of $\delta$ and $\gamma$ for which the posterior model probability was calculated. 
...
To add contours at specific levels of the posterior model probability of calibration, users can specify these levels in a vector via `t_levels`.  Below, we add contours at t = 0.95, 0.9, and 0.8. 
...
Next, we can pass additional arguments to `contour()` via `contour_options`.  Below, we'll draw contours at t = 0.99 and 0.1.  To make them dotted lines instead of solid, we'll use `lty="dotted"` and we'll adjust where the labels are placed via `method="edge"`. 

```{r, fig.width=6, fig.height=4, fig.align='center'}
plot_params(z=zmat_list$z, k=200, dlim=c(0.5, 1.5), glim=c(0.25, 2.75), t_levels = c(0.99, 0.1), 
            contour_options=list(lty = "dotted", method = "edge"))
```
